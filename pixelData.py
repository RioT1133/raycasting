...
class PixelData:
    def __init__(self, width, height):
        self.width = width
        self.height = height
        self.pixels = [random.randint(0, 255) for i in range(3*width*height)]
    ...
    def set_pixel(self, x, y, new_r, new_b, new_g):
        self.pixels[3*self.width*y + 3*x] = new_r
        self.pixels[3*self.width*y + 3*x + 1] = new_b
        self.pixels[3*self.width*y + 3*x + 2] = new_g
